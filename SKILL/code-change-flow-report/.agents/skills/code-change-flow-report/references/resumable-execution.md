...
- `maxWorkerAttempts`: 調査・検証タスクの一時失敗を再試行する上限。既定値は`3`、上限は`20`。APIタイムアウトも1回の一時失敗として記録するが、検証NG後の修正回数には含めない。
...
更新後に初期化した状態は`workerInvocationContractVersion: 1`を持ち、すべての試行へ履歴非継承、ファイルパス入力、成果物参照応答の契約を記録する。更新前の状態は完了済み試行をそのまま再利用できるが、更新後に新しく開始する試行には同じ契約を追加し、担当証跡との一致を検査する。
...
- 新規実行では、親会話を継承していないこと、入力をファイルパスで受け取ったこと、結果を成果物参照だけで返したことを示す`invocationContract`が、開始記録と証跡で一致する。
...
一時失敗または中断は、`maxWorkerAttempts`の範囲で同じ役割を新しいタスクとして再実行する。APIタイムアウトの場合は、元の担当が実行中か、証跡を保存済みかを確認してから失敗を記録する。確認前に代替担当を起動すると同じ出力先を2担当が更新するため禁止する。証跡が存在しても検査が失敗した場合は、その検査失敗を`worker-failed`として記録して実行中状態を解消し、不正な証跡を再利用しない。再試行する新規担当は会話履歴を継承せず、既存のコミット固有入力をファイルパスで受け取る。記録上`running`だが実際には終了しているタスクは、存在を確認したうえで`recover-task`によりそのタスクだけを中断扱いへ戻す。別の実行中タスクを一括で巻き戻さない。すべての実行中タスクが消失したことを確認できた場合だけ`recover-running`を使用できる。
...
同じ調査バッチの複数タスクには開始時刻と終了時刻を記録する。最終ゲートでは、各コミットの初回調査試行の実行区間が実際に重なっていることを確認する。同じバッチ番号を書いただけの逐次実行は拒否する。初回に並列起動した担当の一部だけがタイムアウトした場合、そのコミットだけを再試行してよい。成功済みコミットまで再調査してAPI呼び出しを増やさず、再試行の成功、独立検証、全コミットの完了を別条件として確認する。
...
1. `status`を読む。
2. 実行可能な調査と検証を、利用可能枠まで会話履歴非継承で起動する。修正は利用可能なら元の隔離済み調査担当へ追加依頼し、元の担当を再開できない場合だけ履歴非継承の代替担当を起動する。
3. 各タスク結果を到着順に状態へ保存する。
4. 一時失敗は担当の生存状態と証跡を確認し、重複実行を防いでから上限内で再試行する。
